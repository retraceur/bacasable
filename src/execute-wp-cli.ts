/**
 * Execute a WP-CLI command within bacÀsable.
 *
 * @module execute-wp-cli
 * @since 0.9.0
 */
...
export async function executeWPCli(
	...
	const options = await getBacAsableConfig({
		...
		retraceur: DEFAULT_RETRACEUR_VERSION,
		path: projectPath,
	});
	...
	const createFiles = {
		...
		[runCliPath]: `<?php
			...
			// Force disabling Retraceur debugging mode to avoid parsing issues of WP-CLI command result
