/**
 * Start the server for bacÀsable.
 *
 * @module start-server
 * @since 0.9.0
 */
...
import fs from 'fs';
import { BacAsableOptions } from './config';
...
export async function startServer(
	...
	const { php, options: wpNowOptions } = await startBacAsable(options);
	...
	app.use('/', async (req, res) => {
		try {
			...
			if (isWebContainer()) {
				...
				// URL. However, Retraceur detects when the Host header is different
				// from the stored site URL and redirects back to the site URL.
				// For Retraceur to work, we need to make sure the host and origin
