...
import getBacAsableConfig, { CliOptions } from './config';
...
function commonParameters(yargs) {
	return yargs
		.option('path', {
			describe:
				'Path to the PHP or Retraceur project. Defaults to the current working directory.',
			type: 'string',
		})
		.option('php', {
...
export async function runCli() {
	return yargs(hideBin(process.argv))
		...
		.check(async (argv) => {
			...
			if (argv._[0] !== 'php') {
				config.retraceur = argv.retraceur as string;
				config.port = argv.port as number;
			}
			try {
				await getBacAsableConfig(config);
			} catch (error) {
		...
		.command(
			...
			(yargs) => {
				...
				yargs.option('retraceur', {
					describe: "Retraceur version to use: e.g. '--retraceur=1.0.0'",
					type: 'string',
				});
				yargs.option('port', {
			...
			async (argv) => {
				...
				try {
					const options = await getBacAsableConfig({
						path: argv.path as string,
						php: argv.php as SupportedPHPVersion,
						retraceur: argv.retraceur as string,
